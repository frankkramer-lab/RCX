...
In those cases it might be helpful to read the CX network in a "raw" format, and convert this data in a second step.
...
rcx = fromCX(cx)
```
- read the CX with JSON parsing
```{r readRCXraw, eval=FALSE}
cxParsed = readRCX(cxFile, raw=TRUE)

# OR:

cxParsed = fromCX(cx, raw=TRUE)

# do some manual modification of the data
